...
if (args.Length < 2)
{
    Console.WriteLine("Usage: Vb6ModernConverter <source_dir> <destination_dir> [--target csharp|vbnet] [--config conversion.json] [--remove-line-numbers]");
    return 1;
}
...
for (var i = 2; i < args.Length; i++)
{
    ...
    else if (args[i] == "--remove-line-numbers")
    {
...
if (!string.IsNullOrWhiteSpace(configPath) && File.Exists(configPath))
{
    ...
    if (removeLineNumbers) options.RemoveLineNumbers = true;
}
else
{
    options = new ConversionOptions { Target = target, RemoveLineNumbers = removeLineNumbers };
}
